...
{currentTab === "skills" && (
  <div>
    <label className="block text-sm font-medium mb-2">Skills by Category</label>
  ...
  onChange={(e) => updateCategorySkills(catIndex, e.target.value)}
  className="w-full p-2 border rounded-md"
  rows="3"
...
<p className="text-xs text-gray-500 mt-1">
  Type naturally with commas to separate skills. Press Enter for new lines.
  <br />
  Example: "JavaScript, HTML, CSS" or "JavaScript" on each line
    ...
    {resumeData.skillCategories?.length > 0 && (
      <div className="mt-8 p-4 bg-gray-50 rounded-lg">
        <h3 className="text-sm font-medium mb-3">Your skills by category:</h3>
        {resumeData.skillCategories.map((category, index) => (
          <div key={index} className="mb-3">
            <h4 className="text-sm font-semibold text-gray-700">{category.name || "Unnamed Category"}</h4>
