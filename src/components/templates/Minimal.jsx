...
export default function Minimal({ resumeData }) {
        ...
        <section className="mb-5 resume-section">
          ...
          <div className="space-y-4">
            {experience?.map((job, index) => (
              <div key={index} className="bg-white rounded p-4 page-break-inside-avoid">
                <div className="flex flex-col md:flex-row md:justify-between md:items-start">
                  <h3 className="text-base font-bold text-gray-800">
                    {job.position || job.title}
                  </h3>
                  <span className="text-xs font-medium bg-gray-100 text-gray-700 px-2 py-0.5 rounded-sm mt-1 md:mt-0">
            ...
            {education?.map((edu, index) => (
              <div key={index} className="bg-white rounded p-4">
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-xs font-medium bg-gray-100 text-gray-700 px-2 py-0.5 rounded-sm">
        ...
        <section className="mb-5 resume-section">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {projects?.map((project, index) => (
              <div 
                key={index} 
                className="bg-white rounded p-4 page-break-inside-avoid"
              >
                <div className="flex justify-between items-start">
                  <h3 className="text-base font-bold text-gray-800">
                    {project.title}
            ...
            {certificates?.map((cert, index) => (
              <div key={index} className="bg-white rounded p-4">
                <div className="flex justify-between">
                  ...
                  <span className="text-xs font-medium bg-gray-100 text-gray-700 px-2 py-0.5 rounded-sm whitespace-nowrap">
                    {cert.date}
                  </span>
                </div>
                ...
                {cert.url && (
                  <div className="mt-2 text-right">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-xs text-gray-500 hover:text-gray-700 transition-colors underline inline-flex items-center gap-1"
                    >
  ...
  return (
    <div className="bg-gray-50 min-h-full font-sans text-gray-700 relative">
      {/* Header with personal info - no image */}
      ...
      <div className="fixed top-24 right-4 z-10 p-3 bg-white/90 rounded-lg shadow-lg border border-gray-200 backdrop-blur-sm w-48 print:hidden">
        <h3 className="text-sm font-medium text-gray-700 mb-2 text-center border-b pb-1">
          Add Resume Sections
        </h3>
        <div className="space-y-1 max-h-[70vh] overflow-y-auto">
          {availableSections.map((section) => (
            ...
            >
              ...
              {sectionOrder.includes(section.id) && (
                <span className="ml-auto w-5 h-5 flex items-center justify-center rounded-full bg-gray-600 text-white text-xs">
