...
import { FaEnvelope, FaPhone, FaMapMarkerAlt, FaGithub, FaLinkedin, FaGlobe, FaCalendar } from 'react-icons/fa';
...
export default function Spectrum({ resumeData }) {
    const { personalInfo, experience, education, skillCategories, skills, projects, achievements } = resumeData || {};
  ...
  return (
    <div className="bg-white w-full h-full font-sans text-gray-800 flex flex-col">
      ...
      <header style={{ borderTop: `5px solid ${accentColor}` }} className="pt-8 px-10">
        <div className="flex justify-between items-start flex-wrap">
          <div className="mb-4">
            <h1 className="text-4xl font-bold tracking-tight">
              {personalInfo?.name || 'Your Name'}
            </h1>
            {personalInfo?.title && <p className="text-xl text-gray-600 mt-1">{personalInfo.title}</p>}
          </div>
          
          {/* Contact information in a compact grid */}
          <div className="flex flex-col gap-1 text-sm">
            {personalInfo?.email && (
              <a href={`mailto:${personalInfo.email}`} className="flex items-center gap-2 hover:text-gray-600">
            ...
            {personalInfo?.phone && (
              <a href={`tel:${personalInfo.phone}`} className="flex items-center gap-2 hover:text-gray-600">
      ...
      {personalInfo?.summary && (
        <div style={{ backgroundColor: accentColor }} className="mt-6 py-4 px-10 text-white">
          ...
          <div className="flex-grow md:w-7/12">
            ...
            {experience?.length > 0 && (
              <section className="mb-8">
                <h2 style={{ color: accentColor }} className="text-lg font-bold uppercase mb-4 border-b pb-1">
                      ...
                      {project.technologies && project.technologies.length > 0 && (
                        <div className="flex flex-wrap gap-2 my-1">
                          {project.technologies.map((tech, idx) => (
                            <span 
                              key={idx} 
                              style={{ backgroundColor: `${accentColor}15` /* 15% opacity */ }}
                ...
                <div className="space-y-4">
                  {skillCategories.map((category, index) => 
                    category.skills.length > 0 && (
                      <div key={index}>
                        <h3 className="font-medium mb-2">{category.name || "Skills"}</h3>
